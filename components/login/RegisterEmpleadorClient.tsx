...
import { Eye, EyeOff, CheckCircle, ArrowLeft, User, Building2, Mail, Lock } from 'lucide-react'
import { createClient } from '@/lib/supabase/client'
import { LoginPageShell } from './LoginPageShell'
import { useTranslations } from 'next-intl'

const inputBase =
  'w-full rounded-lg bg-[#f9fafb] text-[13.5px] text-[#4a4a4a] placeholder:text-[#c0c0c0] outline-none transition-all duration-200 focus:bg-white'
const inputStyle = {
  padding: '11px 14px 11px 40px',
  border: '1.5px solid #e5e7eb',
  borderRadius: 10,
}
const inputFocusClass = 'focus:border-[#2FB7A3] focus:ring-2 focus:ring-[#2FB7A3]/15'
...
function InputWrap({ icon: Icon, children }: { icon: React.ElementType; children: React.ReactNode }) {
  return (
  ...
  const handleGoogle = async () => {
    ...
    try {
      await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: { redirectTo: `${window.location.origin}/api/auth/callback?signup_role=employer` },
      })
    } catch {
    ...
    return (
      <LoginPageShell>
        <div className="flex flex-col items-center text-center gap-5">
          ...
          <Link href="/login" className="flex items-center gap-1.5 text-[13px] text-[#8a8a8a] hover:text-[#2FB7A3] transition-colors">
    ...
    <LoginPageShell>
      ...
      <div className="mb-6">
        ...
        <h2 className="text-[26px] font-bold text-[#4a4a4a] leading-tight mb-1.5">
          {t('registerEmpleadorTitle')}
        </h2>
        <p className="text-[13.5px] text-[#8a8a8a] leading-snug">
          {t('registerEmpleadorSubtitle')}
        </p>
      </div>
      ...
      <form onSubmit={handleRegister} className="flex flex-col gap-3.5">
          ...
          <div>
            ...
            <InputWrap icon={Building2}>
              <input type="text" value={company} onChange={e => setCompany(e.target.value)}
                required placeholder={t('companyPlaceholder')} autoComplete="organization"
                className={`${inputBase} ${inputFocusClass}`} style={inputStyle} />
            </InputWrap>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`${inputBase} ${inputFocusClass} pr-10`} style={inputStyle}
            />
        ...
        <button type="submit" disabled={loading || googleLoading}
          ...
          {loading && <Spinner />}
          {loading ? t('creatingAccount') : t('createEmpleadorAccount')}
        </button>
      </form>
      ...
      <div className="mt-5 flex flex-col gap-1.5 text-center">
        <p className="text-[13px] text-[#8a8a8a]">
          ...
          <Link href="/login/empleador" className="text-[#2FB7A3] font-semibold hover:underline">
            {t('signInLink')}
          </Link>
        </p>
        <p className="text-[13px] text-[#8a8a8a]">
          {t('registerEmpleadorSwitchText')}{' '}
          <Link href="/registro-terapeuta" className="text-[#2FB7A3] font-semibold hover:underline">
            {t('registerEmpleadorSwitchCta')}
          </Link>
        </p>
      </div>
