...
export default function RegisterTerapeutaClient() {
  ...
  const handleGoogleRegister = async () => {
    ...
    try {
      await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: { redirectTo: `${window.location.origin}/auth/callback?signup_role=therapist` },
      })
    } catch {
  ...
  if (success) {
    return (
      <LoginPageShell
        image="/spa-wellness-terapeuta-ilustracion-vectorial-minimalista.png"
        imageAlt="Terapeuta egresada de Litsea"
      >
        <motion.div
    ...
    >
      <motion.div {...fadeUp(0.45)}>
        ...
        <p className="mt-1.5 text-[0.88rem] text-white/45">
          Crea tu perfil y accede a vacantes en spas de lujo
        </p>
      </motion.div>
      ...
      <form onSubmit={handleRegister} className="flex flex-col gap-3.5">
        <motion.div {...fadeUp(0.67)} className="flex flex-col gap-1.5">
          <label className="text-[10px] font-semibold uppercase tracking-widest text-white/35">Nombre completo</label>
          <input type="text" value={fullName} onChange={e => setFullName(e.target.value)} required
            placeholder="María García López" autoComplete="name" className={inputClass} />
        </motion.div>
        ...
        <motion.div {...fadeUp(0.72)} className="flex flex-col gap-1.5">
          ...
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} required
            placeholder="tu@correo.com" autoComplete="email" className={inputClass} />
        </motion.div>
      ...
      <motion.div {...fadeUp(0.94)} className="mt-6 flex flex-col gap-2 text-center">
        ...
        <p className="text-[12px] text-white/30">
          ...
          <Link href="/registro-empleador" className="text-[#2FB7A3] hover:text-[#3ecfbb] transition-colors font-medium">Regístrate aquí</Link>
        </p>
      </motion.div>
    </LoginPageShell>
  )
